...
describe("JWT Functions", () => {
  ...
  it("should throw an error for an invalid JWT", async () => {
    expect(verifyToken("invalidToken")).rejects.toThrow("invalid JWT token: invalidToken");
  });
  ...
  it("should throw an error for an invalid refresh token", async () => {
    expect(getNewAccessToken("invalidToken")).rejects.toThrow("invalid JWT token: invalidToken");
  });
});